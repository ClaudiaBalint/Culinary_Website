...
if(isset($_POST['add_category_btn'])){
    ...
    if(isset($_POST['allergen']) && !empty($_POST['allergen'])) {
        // Preluăm alergenii selectați din formular
        $selected_allergens = $_POST['allergen'];
        // Transformăm array-ul de alergeni într-un string, separat prin virgulă
        $allergens_str = implode(',', $selected_allergens);
    } else {
        // Dacă nu sunt alergeni selectați, putem seta o valoare implicită sau să lăsăm coloana goală, în funcție de nevoi
        $allergens_str = '-'; // sau altă valoare implicită
    }
    ...
    $recipe_query = "INSERT INTO retete (nume, descriere, ingrediente, pasi, indicatii_opt, id_user, img, status, alergeni, calorii, nr_portii, ore, minute, categorie, users_likes, users_dislikes) VALUES 
    ('$recipe_name', '$recipe_description', '$ingredients_str', '$preparation_steps_str', '$optional_instructions', '$id_utilizator', '$image', '$recipe_status','$allergens_str', '$calories', '$nr_portii','$timp_ore','$timp_minute', '$recipe_category', 0, 0)";

    // Rulăm query-ul
    $recipe_query_run = mysqli_query($con, $recipe_query);

    if($recipe_query_run){
        redirect("add-recipe.php", "Rețetă adăugată cu succes!");
    } else {
        redirect("add-recipe.php", "Ceva nu a funcționat");
    }
}
